...
def modify(infile='out_ver10/db/all_pandas_correct_label.db', outfile='out_ver10/db/pb2a_wiregrid_ver10', plotdir='output_ver10/db/modifyDB/', opts=[]):
    ...
    if base_selection!='' : df_w = df_w.query(base_selection);
    ...
    print(df_w_nanbolo);
    new_bolonames = np.array([ '{}.{}{}'.format(__a,__b,__c) for __a,__b,__c in zip(df_w_nanbolo['pixel_name'],df_w_nanbolo['band'],df_w_nanbolo['bolo_type']) ]);
    print('New bolonames = ',new_bolonames);
    if sum(is_nanbolo)>0 : df_w[is_nanbolo]['bolo_name'] = new_bolonames;
