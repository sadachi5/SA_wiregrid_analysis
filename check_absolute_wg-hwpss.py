...
def check_absolute(ver, outfile='out_check_absolute/check_absolute_verAho', isCorrectHWPenc=True, opts=[], isHWPSS=False):
    ...
    if not isHWPSS:
        ...
        wg_quality_cut   = 'pol_angle>=0'.format(np.pi);
        pass;
    additional_cut = '';
    ...
    if not isHWPSS:
        ...
        database_wiregrid = 'output_{}/db/pb2a_wiregrid_hwpss_v3.db'.format(ver);
        tablename_wiregrid = 'pb2a_focalplane';
    else:
    ...
    # Get pandas
    print('open DB: {}'.format(database_wiregrid))
    ...
    df_all['theta_design_angle'] = deg0to180(df_all['pol_angle'], upper=180.);
    ...
    df_offset = df_offset[abs(df_offset['diff_angle'])<outlier_deg] 
    '''
    ...
    axs[0][1].set_ylabel(r'$\theta_{\mathrm{det,'+mode+'}}$ [deg.]',fontsize=16);
    ...
    # Calculate offset
    print(df_offset['det_offset_x']);
    print(df_offset['det_offset_y']);
    center_offset_x = np.mean(df_offset['det_offset_x']);
    center_offset_y = np.mean(df_offset['det_offset_y']);
    print(f'Center offset (x,y) = ({center_offset_x}, {center_offset_y})')
