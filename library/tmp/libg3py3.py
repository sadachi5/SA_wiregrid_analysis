...
''' # This makes some problem in spt3g_software library
try:
...
def set_dataroot(path):
    ...
    tuningdatadir = os.path.join(dataroot, "tuning_logs")
    missing = 0
    ...
    def loadbolo(self, name=None, start=None, end=None):
        ...
        if not self.bolonames.size==0:
            ...
            if all([(nsamples[comb] == n0) for comb in nsamples]):
                #self.bolotime = np.median(np.vstack(self.bolotimes.values()), axis=0)
    ...
    def loadslowlog(self):
        ...
        if os.path.exists(fn_slowdaqrunlist):
            ...
            if not slowfiles:
                ...
                return
            slowData = load_slow(slowfiles, slowdir='')
...
class WHWPAngle(object):
    ...
    def __init__(self, g3c):
        ...
        #print('dclk:', dclk)
        #print('denc:', denc)
        #print('roughspeed: ', enc_speed0 / self.N_TICK * self.CLK_FREQ)
        ...
        #print('speed:', enc_speed / self.N_TICK * self.CLK_FREQ)
        ...
        gaploc = np.where(clk_cnts_diff > 1000)[0]
        gaplen = np.round(
            (clk_cnts_diff[gaploc]) / dclk /50).astype(int)*50
        ...
        angle0 = 0
        enc_angle = (encoder_cnts + angle0) \
                    * (np.pi * 2 / self.N_TICK)

        irig_ok = (irig_time > bolotime[0] - 10**10) * (irig_time < bolotime[-1] + 10**10) # margin 100 s
        if not irig_ok.all():
            #print('Remove strange timestamp!:',)
            #print(' '.join([core.G3Time(x).GetFileFormatString() for x in irig_time[~irig_ok]]))
            del(x)
        if irig_ok.sum() <= 1:
