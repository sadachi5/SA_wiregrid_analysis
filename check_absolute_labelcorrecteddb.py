...
def check_absolute(outfile='out_check_absolute/check_absolute_labelcorrectedDB'):
    ...
    database_mapping = 'output_ver4/db/pb2a_mapping_corrected_label_v2.db';
    ...
    if df_m is not None : dfmerge = pandas.merge(df_m, df_w, how='left', on='readout_name');
    else                : dfmerge = df_w;
    ...
    bools = dfmerge['bolo_type'].isnull() | dfmerge['pol_angle'].isnull() ;
    ...
    df_base  = df_base0;
    df_base2 = df_base0.query('tau>0.&theta_det_err*180./{}<0.5&pol_angle>=0'.format(np.pi));
    #dfmerge2 = dfmerge.query('tau>0.&theta_det_err*180./{}<0.5&pol_angle>=0'.format(np.pi));
    dfmerge  = dfmerge.query('mislabel==True');
    dfmerge2 = dfmerge.query('tau>0.&theta_det_err*180./{}<0.5&pol_angle>=0'.format(np.pi));
    ...
    # 2D plot
    axs[0][0].plot(df_base[dataname2], df_base[dataname],marker='o', markersize=1.0, linestyle='');
    axs[0][0].plot(df_base2[dataname2], df_base2[dataname],marker='o', markersize=1.0, linestyle='', color='r');
